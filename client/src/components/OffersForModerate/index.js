...
const OffersForModerate = (props) => {
  ...
  const setModerateOfferStatus = (id, userId, status, email, firstName) => confirmAlert({
    title: 'confirm',
    message: 'Are u sure?',
    buttons: [
      {
        label: 'Yes',
        onClick: () => setModerateOfferStatusRequest(id, userId, status, email, firstName),
      },
      {
        label: 'No',
      },
  ...
  return (
    <div className={styles.offersContainer}>
      <div className={styles.offers}>
        {offers.map(({ id, fileName, text, Contest: { contestType, userId }, User: { avatar, firstName, lastName, email, rating } }) => <div key={id} className={styles.offerContainer}>
          ...
          <div className={styles.btnsContainer}>
            <div onClick={() => setModerateOfferStatus(id, userId, 'confirmed', email, firstName)} className={styles.resolveBtn}>Confirm</div>
            <div onClick={() => setModerateOfferStatus(id, userId, 'rejected', email, firstName)} className={styles.rejectBtn}>Reject</div>
          </div>
        </div>)}
      </div>
