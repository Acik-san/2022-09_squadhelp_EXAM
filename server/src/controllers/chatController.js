...
module.exports.addMessage = async (req, res, next) => {
  try {
    ...
    const participants = [userId, Number(recipient)].sort(
      (participant1, participant2) => participant1 - participant2
    );
    const conversations = await Conversation.findAll({
      include: [{ model: User, through: { where: { userId: participants } } }],
    });
    const conversationIdList = await getConversationIdList(conversations);
    const conversationId =
      conversationIdList.length > 0
        ? conversationIdList.pop()
        : await createConversation(participants, User);
    const conversation = await Conversation.findAll({
      where: { id: conversationId },
      include: [{ model: UserToConversation, where: { conversationId } }],
    });
    ...
    const blackList = getArrayOfValues(
      conversation,
      'UserToConversations',
      'blackList'
    );
    const favoriteList = getArrayOfValues(
      conversation,
      'UserToConversations',
      'favoriteList'
    );
...
module.exports.getChat = async (req, res, next) => {
  try {
    ...
    const participants = [userId, Number(interlocutorId)].sort(
      (participant1, participant2) => participant1 - participant2
    );
    const conversations = await Conversation.findAll({
      include: [
        { model: User, through: { where: { userId: participants } } },
...
module.exports.getChats = async (req, res, next) => {
  try {
    ...
    const usersToConversations = await UserToConversation.findAll({
      where: { conversationId: getArrayOfValues(user, 'Conversations', 'id') },
    });
    const messagesData = user
      .flatMap(({ Conversations }) => Conversations)
      .map(({ Messages }) => Messages.pop())
      .map(({ dataValues: { conversationId, sender, body, createdAt } }) => ({
        id: conversationId,
        sender,
        text: body,
        createdAt,
    ...
    usersToConversations
      ...
      .sort((conversationData1, conversationData2) =>
        conversationData1.id === conversationData2.id
          ? conversationData1.userId - conversationData2.userId
          : conversationData1.id - conversationData2.id
      )
      .forEach(({ id, userId, blackList, favoriteList }, i, arr) => {
        if (id === arr[i + 1]?.id) {
    ...
    conversationData
      .concat(messagesData)
      .sort((chatData1, chatData2) => chatData1.id - chatData2.id)
      .forEach((chatData, i, arr) => {
...
module.exports.addChatToBlackList = async (req, res, next) => {
  try {
    const {
      body: { participants, blackListFlag, conversationId },
      tokenData: { userId },
    } = req;
...
module.exports.addChatToFavorite = async (req, res, next) => {
  try {
    const {
      body: { participants, favoriteFlag, conversationId },
      tokenData: { userId },
    } = req;
    await updateChatData(
...
module.exports.updateNameCatalog = async (req, res, next) => {
  try {
    const {
      params: { catalogId },
      body: { catalogName, chats },
    } = req;
    const updatedCatalog = await Catalog.update(
      { catalogName },
      { where: { id: Number(catalogId) }, returning: true }
    );
  ...
  try {
    ...
    const catalogMap = userCatalogs.reduce((acc, { Conversations }, i, arr) => {
      Conversations.forEach(
        ({
          catalogs_to_conversations: {
            dataValues: { catalogId, conversationId },
          },
        }) => {
