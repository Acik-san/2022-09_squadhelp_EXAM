...
const {
  ...
  CONTEST_NAMES: { CONTEST_NAME, CONTEST_TAGLINE, CONTEST_LOGO },
} = CONSTANTS;
  ...
  try {
    ...
    const types =
      contestName === CONTEST_NAME
        ? [NAME_STYLE, TYPE_OF_NAME, INDUSTRY]
        : contestName === CONTEST_TAGLINE
        ? [TYPE_OF_TAGLINE, INDUSTRY]
        : contestName === CONTEST_LOGO
        ? [BRAND_STYLE, INDUSTRY]
        : [INDUSTRY];
...
module.exports.setNewOffer = async (req, res, next) => {
  try {
    const {
      params: { contestId },
      body: { contestType, offerData, customerId },
      tokenData: { userId },
    } = req;
    const obj = {};
...
module.exports.setOfferStatus = async (req, res, next) => {
  const {
    params: { contestId },
    body: { command, offerId, creatorId, orderId, priority },
  } = req;
  let transaction;
  if (command === 'reject') {
    try {
