...
const devContestsFilesPath = path.resolve(
  __dirname,
  '../../public/contestsFiles'
);

const filePath = env === 'production' ? '/var/www/html/images/' : devFilePath;

const contestsFilesPath =
  env === 'production' ? '/var/www/html/contestsFiles' : devContestsFilesPath;
...
const uploadAvatars = multer({ storage: storageImages }).single('file');
