...
const errorsLogsPath =
  env === 'production' ? '/var/www/html/errorsLogs' : devErrorsLogsPath;
...
const job = new CronJob(
  ...
  () => {
    if (
      fs.existsSync(ERRORS_LOG_PATH) &&
      fs.statSync(ERRORS_LOG_PATH).size !== 0
    ) {
      fs.readFile(ERRORS_LOG_PATH, 'utf8', (err, data) => {
        fs.appendFileSync(
          `./errorsLogs/${Date.now()}.json`,
          JSON.stringify(
            JSON.parse(data).map(err => ({
              message: err.message,
              code: err.code,
              time: err.time,
            }))
...
module.exports.errorsLogger = err => {
  ...
  fs.writeFileSync(
    ERRORS_LOG_PATH,
    JSON.stringify([
      ...JSON.parse(fs.readFileSync(ERRORS_LOG_PATH)),
      ...errorData,
    ])
  );
};
